...
do
	...
	then
		...
		then
			...
			/usr/bin/rclone rc operations/deletefile _async=true fs=${RCLONESERVICE}: remote="Backup/${SERVER}/Gooby/${FILENAME}-diff.tar.gz"  --user ${RCLONEUSERNAME} --pass ${RCLONEPASSWORD}
