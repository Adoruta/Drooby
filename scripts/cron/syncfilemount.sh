...
AGE=120	# How many minutes old a file must be before copying/deleting
LOG=${LOGS}/mounter-sync.log
TEMPFILE="/tmp/slowfilesmissing"
...
then
	...
	do
		...
		if [[ ${RUNCHECK} -gt 0 ]]; then
			...
			rclone rc operations/movefile _async=true srcFs=Local: srcRemote="${UPLOADS}/${FILE}" dstFs=${RCLONESERVICE}:${RCLONEFOLDER} dstRemote="${FILE}" --user ${RCLONEUSERNAME} --pass ${RCLONEPASSWORD} --dropbox-chunk-size 150M >> ${APILOG}
